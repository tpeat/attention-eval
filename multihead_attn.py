...
class MultiheadLocalAttentionV3(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        # HACK
        # _, _, h_times_w, _ = agg_value.shape
        # agg_value_reshaped = agg_value.permute(2, 0, 1, 3).reshape(h_times_w, n, c)
        
        print(agg_value.shape)
        print(agg_bias.shape)
        # output = agg_value_reshaped + agg_bias
        output = self.projection(output)
        self.last_size_2d = (h, w)
        return output, local_attn
